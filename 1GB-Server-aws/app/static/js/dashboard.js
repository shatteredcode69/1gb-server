...
function addLogEntry(type, message) {
  ...
  if (terminal) {
    terminal.insertAdjacentHTML('beforeend', `<div class="log-entry ${logClass}">${entry}</div>`);
...
function updateAppProof(current) {
  const appMb = Number(current?.process?.rss_mb ?? 0);
...
async function updateMetrics() {
  try {
    ...
    const status =
      current.memory.percent >= 90 ? 'critical' :
    ...
    if (memVal) memVal.textContent = `${current.memory.used_mb.toFixed(0)} MB`;
    if (memBar) memBar.style.width = `${current.memory.percent}%`;
    if (memInfo) memInfo.textContent =
      `${current.memory.available_mb.toFixed(0)} MB available of ${current.memory.total_mb.toFixed(0)} MB`;
    ...
    if (cpuInfo) cpuInfo.textContent = `${current.cpu.cores} cores`;

    // Disk card
    const diskVal = document.getElementById('disk-value');
    const diskBar = document.getElementById('disk-bar');
    const diskInfo = document.getElementById('disk-info');
    if (diskVal) diskVal.textContent = `${current.disk.used_gb.toFixed(1)} GB`;
    if (diskBar) diskBar.style.width = `${current.disk.percent}%`;
    if (diskInfo) diskInfo.textContent =
      `${current.disk.free_gb.toFixed(1)} GB free of ${current.disk.total_gb.toFixed(1)} GB (${current.disk.path})`;

    // Network card
    const netVal = document.getElementById('net-value');
    const netInfo = document.getElementById('net-info');
    if (netVal) netVal.textContent =
      `${current.network.bytes_sent_mb.toFixed(0)} sent / ${current.network.bytes_recv_mb.toFixed(0)} recv MB`;
    if (netInfo) netInfo.textContent =
      `${current.network.packets_sent} pkts sent • ${current.network.packets_recv} pkts recv`;
    ...
    if (systemDetails) {
      systemDetails.innerHTML = `
        <div class="info-item">
          ...
          <div class="info-value">${current.system.hostname}</div>
        </div>
        <div class="info-item">
        ...
        <div class="info-item gold-ticket">
          ...
          <div class="info-value gold-value">${current.process.rss_mb.toFixed(2)} MB</div>
        </div>
        <div class="info-item">
